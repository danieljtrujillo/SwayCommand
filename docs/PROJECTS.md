...
|---|---|---|
| `loop` | `enabled`, `start`, `end` | The loop region; disabled automatically when `end ≤ start`. Set from the time band with Shift+drag, toggled with LOOP or `L`. |
| `locators` | `id`, `name`, `time`, `color` | Time band markers. Double-click the band to add one; clicking within 6 px of one seeks to it. |
