...
This document covers the environment variables the application reads, the renderer query parameters, the settings file, on-disk data locations, and the complete list of network endpoints. Process model and IPC surface: [ARCHITECTURE.md](ARCHITECTURE.md). System overview: [OVERVIEW.md](OVERVIEW.md).

## Environment variables

All five variables are read in `src/main/main.js` at window creation. Three exist for automated verification (`AKSWAYJ_SHOT`, `AKSWAYJ_SHOT_DELAY`, `AKSWAYJ_PROBE`); the other two preset the renderer's query parameters.
...
| `AKSWAYJ_AUTOPLAY` | Project id | unset | Forwarded to the renderer as the `autoplay` query parameter of `dist/index.html`. |
...
```powershell
# PowerShell: capture the boot screen after 8 s
$env:AKSWAYJ_SHOT = "$PWD\shot.png"; $env:AKSWAYJ_SHOT_DELAY = '8000'; npm start
```

```sh
# POSIX shells: start straight into a performance
AKSWAYJ_AUTOPLAY=<project-id> AKSWAYJ_SCENE=<scene-id> npm start
```
...
|---|---|---|
| `autoplay` | Project id | Any non-empty value bypasses the boot screen: the performance starts immediately with the matching project, or with the first project in picker order when the id matches none. The Doctor still runs in the background and its screen stays reachable (Escape from the performance, Escape again from the picker). |
| `scene` | Scene id | Read only when `autoplay` is present. Auto-VJ is disabled and the engine switches to the named scene via `setScene(scene, 0.3)`. The value is one of the eight ids in the scene registry (`src/renderer/engine/scenes/index.js`): `beams`, `swarm`, `ribbons`, `voxels`, `warp`, `nebula`, `mandelbulb`, `cymatic`. `setScene` ignores an id with no registered creator, so an unknown value leaves the project's opening scene on screen with Auto-VJ still disabled. |
...
One key is currently in use:

| Key | Writer | Reader |
|---|---|---|
| `midiOverrides` | None in the current source: the MIDI module holds learned bindings in memory (`getOverrides` in `src/renderer/midi/midi.js`) and no code path passes them to `settings:set` | Applied once at startup via the MIDI module's `setOverrides`; the value's internal shape is defined in `src/renderer/midi/midi.js` |
...
| Bundled documentation read by the in-application viewer | `README.md` and `docs/*.md` at the package root — the repository root in development, the `resources/app.asar` root when packaged |

In-progress downloads use a `.part` suffix next to the destination and are renamed only on completion.

## Network endpoints

Apart from the Doctor's reachability check at startup, every request below is user-initiated: downloads run only from an explicit fix click on the Doctor screen, never silently.
