...
`scenes/index.js` imports the twenty scene modules in registry order — `beams`, `swarm`, `ribbons`, `voxels`, `warp`, `nebula`, `mandelbulb`, `cymatic`, `spectra`, `vjshader`, `ferrofluid`, `chladni`, `valley`, `lattice`, `willidream`, `naturestomb`, `miraclemile`, `tunnelending`, `wormholept1`, `wormholeend` — and derives two exports: `sceneList`, the array of each module's `meta`, and `creators`, a map from scene id to its `createScene` function. The digit keys `1`–`9` index the **active project's pool**, not this registry, so the registry can grow past the digits; the SCENES bank in the left rail lists all twenty and prints each pooled scene's digit.
...
Binding the warm target during the async compile is the single largest win: on a cold shader cache the first draw of a freshly linked scene cost 170–970 ms per scene on the machine this was measured on (the driver compiling for the HDR target after linking against the canvas); bound, the same first draws cost 5–20 ms. Chromium caches compiled programs on disk, so the cost is paid once per build per machine; later launches link in tens of milliseconds. Two cold-cache costs remain synchronous by nature: the PMREM environment (below) and the driver's GLSL translation inside `createScene` for scenes that compile shaders there.
