...
| Region | Contents |
|---|---|
| Top bar | Wordmark; the project button (opens the project menu: New, Open, Save, Save as, recent files, templates); the transport (play/pause, stop, clock, LOOP); the current scene readout; status pills (`SWAY`/`MIDI`/`KEYS` link, `LOOPBACK`/`LINE`/`GROOVE`/`MUTE` input, fps); the deck buttons SYNTH, RACK, KIT, DOCS, HELP. Inside theDAW it is the SWAY tab's only header, one row: the wordmark; stop, play, loop and the clock; a scene menu named for the loaded scene (New, Open, Save and Save as, then theDAW's GANTASMO scenes, its saved scenes, recent files, the cockpit's other templates and Choose a .sway file); the Sway status key, whose panel holds the device, the audio source, the link to theDAW and the frame rate; the synth, rack, kit and plugins keys as icons; and a ? menu with Docs, Help and the shortcuts |
...
| Bottom band | The timeline: a toolbar (IMPORT, + TRACK, BPM, TAP, SNAP), a head column (SCENES, then one head per audio track with M / S), a song overview strip (the whole song with the visible window drawn on it: drag to seek, drag the window to pan), a time band in bars and beats (scrub, loop region, locators; focused, Left and Right step a beat, Shift a bar, Home and End go to the ends), a VISUAL lane of scene clips, one lane per audio track with waveforms and effect sections, a horizontal scrollbar, and the playhead |
